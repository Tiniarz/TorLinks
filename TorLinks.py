...
def line_print(text, delay, color):
    for line in text.splitlines():
        print(color + line)
        time.sleep(delay)

def human_typing(text, delay, color):
    for char in text:
...
def main():
    while True:
        warning_text = WARNING_ASCII + "\nYour about to see dark web links.\nMake sure you have tor and have your choice on entering them.\nNo one is responsible for anything that happens on the dark web for you.\nYou have your option you hold them and the responsibilty belongs to you.\nContinue?"
        
        line_print(warning_text, 0.5, Fore.YELLOW)
        
        input(Fore.WHITE + "\n[20:53:01] [!] | Press Enter to continue -> ")
        
        human_typing(ASCII_ART, 0.0005, Fore.RED)
        human_typing(LINKS_CONTENT, 0.005, Fore.RED)
        
        input(Fore.WHITE + "\n[20:53:01] [!] | Press Enter to return to warning -> ")
        print("\n" * 2)
